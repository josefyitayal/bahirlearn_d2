...
export default function CoursePage() {
  ...
  return (
    <div>
      ...
      <CourseDialog open={open} onOpenChange={setOpen} setRefreshKey={setRefreshKey} course={selectedCourse} />
      {isLoading ? (
        <div className="w-full text-center text-2xl">Course Loading...</div>
      ): (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {courses.map((course) => (
            <CourseCard key={course.id} course={course} setOpen={setOpen} setSelectedCourse={setSelectedCourse} setRefreshKey={setRefreshKey} />
          ))}
        </div>
      )}
    </div>
